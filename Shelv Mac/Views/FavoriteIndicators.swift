...
struct AlbumFavoriteBadge: View {
    ...
    init(albumId: String, style: DownloadIndicatorStyle = .list) {
        ...
        self.style = style
        _isFavorite = State(
            initialValue: LibraryViewModel.shared.starredAlbums.contains { $0.id == albumId }
        )
    }
...
struct ArtistFavoriteBadge: View {
    ...
    init(artistId: String, style: DownloadIndicatorStyle = .list) {
        ...
        self.style = style
        _isFavorite = State(
            initialValue: LibraryViewModel.shared.starredArtists.contains { $0.id == artistId }
        )
    }
...
struct SongFavoriteBadge: View {
    ...
    init(songId: String, style: DownloadIndicatorStyle = .list) {
        ...
        self.style = style
        _isFavorite = State(
            initialValue: LibraryViewModel.shared.starredSongs.contains { $0.id == songId }
        )
    }
