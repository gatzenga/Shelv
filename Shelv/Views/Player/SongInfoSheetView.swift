...
struct SongInfoSheetView: View {
    ...
    @State private var selectedTab: SongInfoTab
    @State private var isLoading = false

    init(song: Song, initialTab: SongInfoTab = .credits) {
        self.song = song
        self._displayedSong = State(initialValue: song)
    ...
    private var header: some View {
        HStack(spacing: 14) {
            AlbumArtView(coverArtId: displayedSong.coverArt, size: 180, cornerRadius: 10)
    ...
    private var creditsContent: some View {
        ...
        if sections.isEmpty {
            emptyState(
                icon: "person.2.slash",
                title: String(localized: "song_info_no_credits")
            )
        } else {
            List {
    ...
    private var detailsContent: some View {
        ...
        if sections.isEmpty {
            emptyState(
                icon: "info.circle",
                title: String(localized: "song_info_no_details")
            )
        } else {
            List {
                ForEach(sections) { section in
    ...
    private func refreshSong() async {
        ...
        do {
            ...
            // Keep the already playing song metadata when the server cannot refresh details.
        }
    }
}

enum SongInfoTab: CaseIterable, Identifiable {
    case credits
    case details
...
private extension SongInfoSheetView {
