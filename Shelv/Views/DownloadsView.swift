...
struct DownloadsView: View {
    ...
    enum Segment: String, CaseIterable {
        ...
        var label: String {
            switch self {
            case .albums:    return tr("car.play.car.play.library.albums")
            case .artists:   return tr("car.play.car.play.library.artists")
            case .favorites: return tr("car.play.car.play.library.favorites")
            }
        }
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(offlineMode.isOffline ? tr("downloads.offline") : tr("car.play.car.play.library.downloads"))
    ...
    private var albumsList: some View {
        if downloadStore.albums.isEmpty {
            emptyState(title: tr("downloads.no_downloaded_albums"),
                        ...
                        LazyVGrid(columns: [GridItem(.adaptive(minimum: 130, maximum: 180), spacing: 14)], spacing: 18) {
                            ForEach(downloadStore.albums) { album in
                                ...
                                .contextMenu {
                                    ...
                                    } label: { Label(tr("downloads.delete_downloads.d9dd6fd8"), systemImage: "trash") }
                                }
                            }
                        }
                        .padding(16)
    ...
    private var artistsList: some View {
        if downloadStore.artists.isEmpty {
            emptyState(title: tr("downloads.no_downloaded_artists"),
                       icon: "music.mic")
        } else {
            List {
    ...
    private var favoritesList: some View {
        ...
        if favAlbums.isEmpty && favArtists.isEmpty && favSongs.isEmpty {
            emptyState(title: tr("downloads.no_favorites_downloaded"),
                       icon: "heart")
        } else {
            List {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("\(artist.albumCount) \(tr("car.play.car.play.library.albums"))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Spacer()
