...
struct RecapDetailView: View {
        ...
        Group {
            if isLoading {
                ...
                ContentUnavailableView(
                    tr("recap.recap.detail.no_songs"),
                    systemImage: "music.note",
                    description: Text(tr("recap.recap.detail.no_songs_found_period"))
                )
            } else {
                List {
                    Section {
            ...
            ToolbarItem(placement: .topBarTrailing) {
                Menu {
                    ...
                    Button(role: .destructive) {
                        ...
                        Label(tr("recap.recap.detail.delete_recap"), systemImage: "trash")
                    }
                } label: {
                ...
                Task {
                    do {
                        ...
                        if !(error is CancellationError) {
                            currentToast = ShelveToast(message: tr("recap.recap.detail.could_not_delete_recap"), isError: true)
                        }
                    }
                }
            }
    ...
    private func downloadHeaderButtons() -> some View {
        ...
        let totalCount = downloadStore.playlistSongIds[entry.playlistId]?.count ?? allSongs.count
        let remaining = isMarked ? max(0, totalCount - downloadStore.downloadedCount(for: entry.playlistId)) : 0
        HStack(spacing: 10) {
    ...
    private func load() async {
        ...
        guard let playlist = await libraryStore.loadPlaylistDetail(id: entry.playlistId) else {
            errorMessage = tr("recap.recap.detail.playlist_could_not_loaded")
            return
        }
