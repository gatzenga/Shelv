...
struct BulkDownloadSheet: View {
            ...
            Section {
                ...
                if mode.isKeepLibraryOffline {
                    ...
                    if let downloaded = plan.downloadedBytes, downloaded > 0 {
                        HStack {
                            Text(String(localized: "already_downloaded")).font(.subheadline)
                            Spacer()
    ...
    private func recompute() async {
        ...
        let cachedAll = await LibraryRepository.shared.cachedAlbums(
            serverKey: serverID.uuidString,
            libraryIDs: selection.allCacheFolderIDs
        )
        let albums = cachedAll.isEmpty ? libraryStore.albums : cachedAll
