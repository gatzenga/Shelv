...
struct DiscoverView: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 28) {
                    if offlineMode.isOffline {
                        ...
                        VStack(spacing: 20) {
                            ...
                            if showOfflineHint {
                                Button {
                                    ...
                                    Label(tr("discover.go_offline"), systemImage: "wifi.slash")
                        ...
                        VStack(spacing: 12) {
                            mixButton(
                                title: tr("car.play.car.play.discover.mix_newest_tracks"),
                                icon: "sparkles",
                                key: "newest"
                            ) { await loadMix(type: "newest") }

                            mixButton(
                                title: tr("car.play.car.play.discover.mix_frequently_played"),
                                icon: "chart.bar.fill",
                                key: "frequent"
                            ) { await loadMix(type: "frequent") }

                            mixButton(
                                title: tr("car.play.car.play.discover.mix_recently_played"),
                                icon: "clock.fill",
                                key: "recent"
                        ...
                        albumSection(
                            title: tr("discover.recently_added"),
                            albums: libraryStore.recentlyAdded
                        )
                        albumSection(
                            title: tr("discover.recently_played"),
                            albums: libraryStore.recentlyPlayed
                        )
                        albumSection(
                            title: tr("car.play.car.play.discover.frequently_played"),
    ...
    private var offlineEmptyState: some View {
        VStack(spacing: 18) {
            ...
            Text(tr("car.play.car.play.discover.offline"))
                .font(.title3).bold()
            Text(tr("discover.downloads_still_available_tap_magnifying_glass"))
            .font(.subheadline)
            ...
            Button {
                ...
                Label(tr("car.play.car.play.discover.go_online"), systemImage: "wifi")
                    .font(.subheadline.bold())
                    .padding(.horizontal, 24)
                    .padding(.vertical, 10)
                    .background(accentColor.opacity(0.15))
    ...
    private var offlineMixState: some View {
        VStack(alignment: .leading, spacing: 0) {
            VStack(spacing: 12) {
                mixButton(
                    title: tr("discover.play_downloads"),
                    icon: "play.fill",
                    key: "offline_play"
                ) { loadOfflineMix(type: "offline_play") }

                mixButton(
                    title: tr("discover.shuffle_downloads"),
                    icon: "shuffle",
                    key: "offline_shuffle"
                ) { loadOfflineMix(type: "offline_shuffle") }

                mixButton(
                    title: tr("discover.mix_latest_downloads"),
    ...
    private var randomAlbumSection: some View {
        albumSection(title: tr("discover.random_albums"), albums: libraryStore.randomAlbums) {
            Button {
