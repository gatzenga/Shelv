...
enum SubsonicAPIError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noServer:              return tr("services.subsonic.api.service.no_server_configured")
        case .noPassword:            return tr("services.subsonic.api.service.no_password_found")
        case .invalidURL:            return tr("services.subsonic.api.service.invalid_server_url")
        case .networkError(let e):
            if let urlError = e as? URLError {
                switch urlError.code {
                case .timedOut:
                    return tr("services.subsonic.api.service.connection_timed_out_please_check_network")
                case .notConnectedToInternet:
                    return tr("services.subsonic.api.service.no_internet_connection")
                case .cannotConnectToHost, .cannotFindHost:
                    return tr("services.subsonic.api.service.server_not_reachable_please_check_url")
                case .networkConnectionLost:
                    return tr("services.subsonic.api.service.connection_lost_please_try_again")
                case .secureConnectionFailed:
                    return tr("services.subsonic.api.service.secure_connection_failed_please_check_server")
                default:
                    return tr("services.subsonic.api.service.network_error_please_try_again")
                }
            }
            return tr("services.subsonic.api.service.network_error_please_try_again")
        case .apiError(_, let m):
            return m ?? tr("services.subsonic.api.service.server_returned_error")
        case .decodingError:
            return tr("services.subsonic.api.service.unexpected_server_response")
        }
    }
}
